...
def generate_grades():
  ...
  while True:
    try:
      num_grades = int(input("Enter a integer between 3 and 20 inclusive. "))
    except: 
      print("invalid data type -- enter an integer")
    else:
      if (3 <= num_grades <= 20): #checking if int is in range 
        break
      else:
        print("integer not in range 3-20.")
